...
test("shows English by default, has contact paths, and can switch to Portuguese", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("heading", { name: /signals searching for meaning/i })).toBeVisible({ timeout: 20_000 });
  ...
  await expect(page.getByRole("heading", { name: /engineering learns to care/i })).toBeVisible();
  await expect(page.getByText("Concept development and validation")).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: /engenharia aprende a cuidar/i })).toBeVisible();
});
